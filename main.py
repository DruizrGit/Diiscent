...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Background Images
        self.woodland_image = ImageTk.PhotoImage(Image.open("Images/Woodland.jpg").resize((controller.width,controller.height)))
        ...
        for colour in self.atk_colours:
            self.atk_dice_images[colour] = ImageTk.PhotoImage(Image.open("Images/Descent" +  colour + "Dice.png").resize(die_dims))
            if colour == 'Red':
                self.dice_cube_images[colour] = ImageTk.PhotoImage(Image.open("Images/" + colour + "Cube.png").resize((18,18)))
            else:
                self.dice_cube_images[colour] = ImageTk.PhotoImage(Image.open("Images/" + colour + "Cube.png").resize(die_cube_dims))
        for colour in self.def_colours:
            self.def_dice_images[colour] = ImageTk.PhotoImage(Image.open("Images/Descent" +  colour + "Dice.png").resize(die_dims))
            if colour == 'Brown':
                self.dice_cube_images[colour] = ImageTk.PhotoImage(Image.open("Images/" + colour + "Cube.png").resize((22,22)))
            else:
                self.dice_cube_images[colour] = ImageTk.PhotoImage(Image.open("Images/" + colour + "Cube.png").resize(die_cube_dims))
        ...
        # Attack Dice Buttons
        self.blue_dice = ImageTk.PhotoImage(Image.open("Images/DescentBlueDice.png").resize((die_length,die_length)))
        blue_dice_button = tk.Button(self, image = self.blue_dice, bg = 'blue', command = lambda: self.update_atk_dice('Blue', False))
        self.canvas.create_window(self.atk_x, self.atk_y, anchor = tk.NW, window = blue_dice_button)
        
        self.yellow_dice = ImageTk.PhotoImage(Image.open("Images/DescentYellowDice.png").resize((die_length,die_length)))
        yellow_dice_button = tk.Button(self, image = self.yellow_dice, bg = 'yellow', command = lambda: self.update_atk_dice('Yellow', False))
        self.canvas.create_window(self.atk_x + self.atk_sep, self.atk_y, anchor = tk.NW, window = yellow_dice_button)
        
        self.red_dice = ImageTk.PhotoImage(Image.open("Images/DescentRedDice.png").resize((die_length,die_length)))
        red_dice_button = tk.Button(self, image = self.red_dice, bg = 'red', command = lambda: self.update_atk_dice('Red', False))
        self.canvas.create_window(self.atk_x + 2*self.atk_sep, self.atk_y, anchor = tk.NW, window = red_dice_button)
        
        self.green_dice = ImageTk.PhotoImage(Image.open("Images/DescentGreenDice.png").resize((die_length,die_length)))
        green_dice_button = tk.Button(self, image = self.green_dice, bg = 'green', command = lambda: self.update_atk_dice('Green', False))
        self.canvas.create_window(self.atk_x + 3*self.atk_sep, self.atk_y, anchor = tk.NW, window = green_dice_button)
    
        # Defense Dice Buttons
        self.black_dice = ImageTk.PhotoImage(Image.open("Images/DescentBlackDice.png").resize((die_length,die_length)))
        black_dice_button = tk.Button(self, image = self.black_dice, bg = 'black', command = lambda: self.update_def_dice('Black', False))
        self.canvas.create_window(self.def_x, self.def_y, anchor = tk.NW, window = black_dice_button)
        
        self.grey_dice = ImageTk.PhotoImage(Image.open("Images/DescentGreyDice.png").resize((die_length,die_length)))
        grey_dice_button = tk.Button(self, image = self.grey_dice, bg = 'grey', command = lambda: self.update_def_dice('Grey', False))
        self.canvas.create_window(self.def_x + self.def_sep, self.def_y,  anchor = tk.NW, window = grey_dice_button)
        
        self.brown_dice = ImageTk.PhotoImage(Image.open("Images/DescentBrownDice.png").resize((die_length,die_length)))
        brown_dice_button = tk.Button(self, image = self.brown_dice, bg = 'brown', command = lambda: self.update_def_dice('Brown', False))
        self.canvas.create_window(self.def_x + 2*self.def_sep, self.def_y,  anchor = tk.NW, window = brown_dice_button)
        
        
        # Roll (Plotting) Button
        self.roll_image = ImageTk.PhotoImage(Image.open("Images/Roll.png"))
        plot_button = tk.Button(self, image = self.roll_image, bg = 'black', takefocus = False, command = self.draw_figures)
        ...
        self.reset_button = tk.Button(self, image = self.reset_image, bg = 'black', takefocus = False, command = self.clear_pool)
        ...
        greater_check_atk = tk.Radiobutton(self, image = self.greater_text, bg = 'black', var = self.fig_order_atk, indicatoron = 0, value = 'great')
        
        self.equal_text = ImageTk.PhotoImage(Image.open("Images/Equal.png"))
        equal_check_atk = tk.Radiobutton(self, image = self.equal_text, bg = 'black', var = self.fig_order_atk, indicatoron = 0, value = 'exact')
        
        self.less_text = ImageTk.PhotoImage(Image.open("Images/Less.png"))
        lesser_check_atk = tk.Radiobutton(self, image = self.less_text, bg = 'black', var = self.fig_order_atk, indicatoron = 0, value = 'less')
        ...
        greater_check_def = tk.Radiobutton(self, image = self.greater_text, bg = 'black', var = self.fig_order_def, indicatoron = 0, value = 'great')
        
        equal_check_def = tk.Radiobutton(self, image = self.equal_text, bg = 'black', var = self.fig_order_def, indicatoron = 0, value = 'exact')
        
        lesser_check_def = tk.Radiobutton(self, image = self.less_text,bg = 'black', var = self.fig_order_def, indicatoron = 0, value = 'less')
        ...
        icon_size = 40
        self.melee_icon = ImageTk.PhotoImage(Image.open("Images/MeleeIcon40px.png").resize((icon_size, icon_size)))
        self.range_icon = ImageTk.PhotoImage(Image.open("Images/RangeIcon40px.png").resize((icon_size, icon_size)))
        
        self.kind_button = tk.Button(self, image = self.melee_icon, command = self.toggle_dmg_range, border = 0)
    ...
    def show_menubar(self, controller):
        ...
        self.helptab.add_command(label = "Dice Breakdown", command = lambda: self.show_dice_breakdown(controller))
    ...
    def show_dice_breakdown(self, controller):
        ...
        self.dicebreakdown_image = ImageTk.PhotoImage(Image.open("Images/DiceBreakdown.png"))
    ...
    def draw_atk_probs(self, kind = 'Heart', measure = 'great'):
        ...
        if kind == 'Heart':
            ...
            for val in range(maxh+1):
                probs.append(atk_pool.event_select(heart_val = val, heart_measure = measure)['Prob']*100)
            ...
            for rect in bar:
                ...
                graph.text(rect.get_x() + rect.get_width()/2, height, str(atk_pool.fraction_rounder(probs[ind])), 
                         ha = 'center', va = 'bottom', size = bar_text_size)
                ind+=1
            ...
            graph.set_title('Probability of Rolling ' + str_kind + ' x Hearts', y = 1.08, size = 12)
            ...
            for val in range(maxr+1):
                probs.append(atk_pool.event_select(range_val = val, range_measure = measure)['Prob']*100)
            ...
            for rect in bar:
                height = rect.get_height()
                graph.text(rect.get_x() + rect.get_width()/2, height, str(atk_pool.fraction_rounder(probs[ind])), ha = 'center', va = 'bottom', 
                         size = bar_text_size)
                ind+=1
            ...
            graph.set_xticks(x_axis)
            graph.set_xticklabels(labels = values, size = xtick_text_size)
            graph.set_title('Probability of Rolling ' + str_kind + ' x Range', y = 1.08, size = 12)
            graph.set_ylim(0,100)
    ...
    def draw_atk_graph_dice(self):
        ...
        for colour in self.atk_colours:
            ...
            for die in range(self.atk_state[colour][0]):
                ...
                atk_die_label = tk.Label(self.atk_die_frame, image = self.dice_cube_images[colour])
    ...
    def draw_def_probs(self, measure = 'great'):
        ...
        for val in range(maxs+1):
            probs.append(def_pool.event_select(shield_val = val, shield_measure = measure)['Prob']*100)
        ...
        for rect in bar:
            height = rect.get_height()
            graph.text(rect.get_x() + rect.get_width()/2, height, str(def_pool.fraction_rounder(probs[ind])), 
                     ha = 'center', va = 'bottom', size = bar_text_size)
            ind+=1
    ...
    def draw_def_graph_dice(self):
        ...
        for colour in self.def_colours:
            ...
            for die in range(self.def_state[colour][0]):
                ...
                def_die_label = tk.Label(self.def_die_frame, image = self.dice_cube_images[colour])
    ...
    def create_atk_buttons(self):
        ...
        for colour in self.atk_colours:
            ...
            for dice in range(self.atk_state[colour][0]):
                ...
                self.atk_state[colour][1].append(tk.Button(self.attack_pool, image = self.atk_dice_images[colour], padx=5,pady = 5, bg = colour, command = lambda colour=colour: self.update_atk_dice(colour, True)))
    ...
    def create_def_buttons(self):
        ...
        for colour in self.def_colours:
            ...
            for dice in range(self.def_state[colour][0]):
                ...
                self.def_state[colour][1].append(tk.Button(self.defense_pool, image = self.def_dice_images[colour], bg = colour, command = lambda colour=colour: self.update_def_dice(colour, True)))
